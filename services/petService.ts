...
export const petService = {
  ...
  async getPetById(
    ...
    const { data: bodyConditionLogs, error: bodyConditionError } =
      await supabase
  ...
  async updatePet(
    id: string,
    updates: Partial<
      Omit<Pet, "logs_weight" | "logs_bodycondition" | "logs_vet_visits">
    >
  ): Promise<Pet> {
    const { data, error } = await supabase
      .from("pets")
  ...
  async updateWeightLog(
    id: string,
    updates: Partial<Omit<WeightLog, "id">>
  ): Promise<WeightLog> {
    const { data, error } = await supabase
      .from("weight_logs")
      .update(updates)
  ...
  async addBodyConditionLog(
    bodyConditionLog: Omit<BodyConditionLog, "id">
  ...
  async updateBodyConditionLog(
    id: string,
    updates: Partial<Omit<BodyConditionLog, "id">>
  ): Promise<BodyConditionLog> {
    const { data, error } = await supabase
      .from("body_condition_logs")
      .update(updates)
  ...
  // Vet Visit Logs
  async addVetVisitLog(
    vetVisitLog: Omit<VetVisitLog, "id">
  ): Promise<VetVisitLog> {
    const { data, error } = await supabase
      .from("vet_visit_logs")
      .insert(vetVisitLog)
  ...
  async updateVetVisitLog(
    id: string,
    updates: Partial<Omit<VetVisitLog, "id">>
  ): Promise<VetVisitLog> {
    const { data, error } = await supabase
      .from("vet_visit_logs")
      .update(updates)
